				...
				<ul class="navItems">
					...
					<?php
						...
						for ($i=0; $i < sizeof($navItems) ; $i++) {
							...
							$newNav['pageTitle'] = $temp[4];
							$newNav['metaDescription'] = $temp[5];
						...
						for ($i=0; $i < sizeof($navObjects); $i++) {
							...
							'<li class="navItem nav_'.$i.'">'.chr(13).
								...
								'<input class="linkParams" value="'.$navObjects[$i]['parameters'].'" id="parameters'.$i.'" data-id="'.$i.'" type="text" />'.chr(13).
								'<label for="">Page Title:</label>'.chr(13).
								'<input value="'.$navObjects[$i]['pageTitle'].'" id="pageTitle'.$i.'" data-id="'.$i.'" type="text" />'.chr(13).
								'<label for="">Meta Description:</label>'.chr(13).
								'<textarea value="" id="metaDescription'.$i.'" data-id="'.$i.'" />'.$navObjects[$i]['metaDescription'].'</textarea>'.chr(13).
								'<input id="hidden'.$i.'" data-id="'.$i.'" type="hidden" name="navOptions[]" value="'
									.$navObjects[$i]['title'].'|'
									.$navObjects[$i]['type'].'|'
									.$navObjects[$i]['target'].'|'
									.$navObjects[$i]['parameters'].'|'
									.$navObjects[$i]['pageTitle'].'|'
									.$navObjects[$i]['metaDescription'].
								'" />'.chr(13).
							'</li>';
						}
